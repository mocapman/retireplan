...
def diagnose(cfg_path: str, years_to_show: int = 15) -> int:
    ...
    print("\n=== Early-year budget breakdown (gross-budget model) ===")
    print(
        "Year  Living   Filing  Budget  Taxes  Events  Discret  SS  Draw_IRA  Draw_Brk  Draw_Roth  Conv  "
    ...
    for idx, yc in enumerate(years[:years_to_show]):
        ...
        need_total = r["Spend_Target"] + r["Taxes"]  # events are inside budget already
        provided = (
            r["SS_Income"]
            + r["RMD"]
            + r["Draw_IRA"]
        ...
        print(
            ...
            f"{int(r['Spend_Target']):6d}  {int(r['Taxes']):5d}  {int(r['Events_Cash']):6d}  {int(r['Discretionary_Spend']):7d}  "
            f"{int(r['SS_Income']):5d}  {int(r['Draw_IRA']):8d}  {int(r['Draw_Brokerage']):8d}  {int(r['Draw_Roth']):9d}  "
            f"{int(r['Roth_Conversion']):4d}  "
            f"{int(need_total):7d}  {int(provided):8d}  {int(r['Shortfall']):9d}  "
            f"{int(r['MAGI']):5d}  {int(target_magi):10d}"
        )

    print("\n=== Pre-Medicare MAGI fill status ===")
    print("Year  Age  MAGI  Target  FillDelta  Status")
    for idx, yc in enumerate(years[:years_to_show]):
        r = rows[idx]
        if r["Age_You"] >= cfg.aca_end_age:
            continue
        target = cfg.magi_target_base * _infl(cfg.inflation, idx)
        delta = target - r["MAGI"]
        status = "OK" if abs(delta) <= 1 else ("GAP" if delta > 1 else "OVER")
        print(
            f"{int(r['Year']):4d}  {int(r['Age_You']):3d}  {int(r['MAGI']):5d}  {int(target):6d}  {int(delta):8d}  {status}"
        )
