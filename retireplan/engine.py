...
def run_plan(cfg, events: Iterable[dict] | None = None) -> list[dict]:
    ev_by_year = {}
    for e in events or []:
        ev_by_year.setdefault(e["year"], []).append(e)
    ...
    for idx, yc in enumerate(years):
        ...
        ss_sp = (
            ...
            if cfg.birth_year_spouse
            else 0.0
        )
        ss_inc = ss_you + ss_sp

        # events impact (positive = spend increase, negative = inflow)
        ev_amt = 0.0
        for e in ev_by_year.get(yc.year, []):
            ev_amt += float(e.get("amount", 0.0))

        # taxes placeholder (v0): 0
        taxes = 0.0

        # need after SS and events plus taxes
        need = max(0.0, spend + taxes + ev_amt - ss_inc)

        # draw order
        d_b, d_r, d_i = accts.withdraw_sequence(need)

        # no conversions or RMD yet in v0
        roth_conv = 0.0
        rmd = 0.0
        magi = 0.0
        std_ded = cfg.standard_deduction_base  # placeholder fixed base
